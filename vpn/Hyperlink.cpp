...
CHyperlink::CHyperlink()
	...
	m_bMatchParentFont = FALSE;	// Assume the user wants to initially match the parent font.
	m_nAlignStyle = ALIGN_CENTER;	// Assume the user wants to align the text on the left.
	...
	SetHyperlinkFont(lf.lfFaceName, lf.lfHeight, lf.lfCharSet, lf.lfItalic, bBold);
	...
	m_bAlwaysUnderlineText = FALSE;	// Assume we always want underlining.
...
// ----------------------------------------------------------------------------------------------------
void CHyperlink::SetHyperlinkFont(LPCTSTR strFontName, long nFontHeight,  BYTE lfCharSet, BOOL bItalics, BOOL bBold)
	{
...
// ----------------------------------------------------------------------------------------------------
void CHyperlink::SetHyperlinkFont(LPCTSTR strFontName, int nPoints, BYTE lfCharSet, BOOL bItalics, BOOL bBold)
	{
	LOGFONT lf;											// Declare a logfont object.
	memset(&lf, 0, sizeof(LOGFONT));					// Clear the logfont structure.
	_tcscpy_s(lf.lfFaceName, LF_FACESIZE, strFontName);	// Font name passed by the user.
...
// ----------------------------------------------------------------------------------------------------
// Get the current hyperlink font's charset.
// ----------------------------------------------------------------------------------------------------
int CHyperlink::GetHyperlinkFontCharSet()
	{
	LOGFONT lf;
	memset(&lf, 0, sizeof(LOGFONT));
	if (&m_Font != NULL)
